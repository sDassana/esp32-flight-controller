...
unsigned long lastAckRefresh = 0;  // For periodic ACK payload refresh
bool controlDataReceived = false;  // Track if we've received any control data
...
void loop()
{
    ...
    // COMMENTED OUT: Periodic ACK refresh that might interfere with incoming transmissions
    /*
    // Refresh ACK payload periodically (every 1 second) to ensure buffer stays ready
    if (millis() - lastAckRefresh > 1000)
    {
        // Only refresh if we haven't received control data recently (to avoid conflicts)
        if (!radio.available() && !controlDataReceived)
        {
            // Clear buffers before attempting to load ACK payload
            radio.flush_tx();
            delay(5);

            bool refreshResult = radio.writeAckPayload(1, &telemetryData, sizeof(telemetryData));
            Serial.print("ACK payload refreshed: ");
            if (refreshResult)
            {
                Serial.println("OK");
            }
            ...
            {
                Serial.println("FAILED - attempting radio recovery");
                // Try to recover radio
                radio.powerUp();
                radio.startListening();
                delay(20);
                // Try again
                refreshResult = radio.writeAckPayload(1, &telemetryData, sizeof(telemetryData));
                Serial.print("ACK payload retry: ");
                Serial.println(refreshResult ? "SUCCESS" : "STILL FAILED");
            }
        }
        lastAckRefresh = millis();
    }
    */

    // Print periodic status (every 10 seconds to reduce spam)
    if (millis() - lastStatusPrint > 10000)
    {
        Serial.print("Drone status - Running for ");
        Serial.print(millis() / 1000);
        Serial.print("s, Radio available: ");
        Serial.print(radio.available());
        Serial.print(", Channel: ");
        Serial.print(radio.getChannel());
        Serial.print(", Control received: ");
        Serial.println(controlDataReceived ? "YES" : "NO");
        lastStatusPrint = millis();

        // Reset the control received flag for next period
        controlDataReceived = false;
    }

    delay(10); // Reduced delay for faster response to incoming packets
}
...
void initializeRadio()
{
    ...
    // Wait for radio to settle before loading ACK payload
    delay(200);

    // Pre-load an initial ACK payload (this is critical for first transmission)
    bool initialAckResult = radio.writeAckPayload(1, &telemetryData, sizeof(telemetryData));
    Serial.print("Initial ACK payload loaded: ");
    Serial.println(initialAckResult ? "SUCCESS" : "FAILED");
    ...
    Serial.println("Drone Address Configuration:");
...
void handleControlData()
{
    if (radio.available())
    {
        ...
        controlDataReceived = true;

        // Process control data
        processControlInputs();

        // Immediately load ACK payload for response
        ...
        {
            Serial.println("Control packet received, ACK sent.");
            printTelemetryData();
            lastStatusPrint = millis();
        }
    }
